...
def main():
    ...
    ok = xrr[xrr.run.isin([24, 26, 30, 32, 33])].sort_values('run')
    fig, (a1, a2) = plt.subplots(1, 2, figsize=(10.0, 3.5),
    ...
    growth_time_min = 10  # 10-min growth
    x2 = np.arange(len(t))
    
    # Calculate growth rates in nm/min
    growth_rates = [v[1] / growth_time_min for v in t]
    growth_rate_errs = [v[2] / growth_time_min for v in t]
    
    a2.bar(x2, growth_rates, yerr=growth_rate_errs, width=0.55, color=TEAL,
           error_kw=dict(ecolor=INK, lw=1.6, capsize=4))
    for xi, gr in zip(x2, growth_rates):
        a2.text(xi, gr + 0.4, f'{gr:.1f}', ha='center', fontsize=16.5, color=INK)
    a2.set_xticks(x2)
    a2.set_xticklabels([v[0] for v in t], fontsize=17)
    a2.set_ylim(0, 15)
    a2.set_ylabel('growth rate (nm min$^{-1}$)', fontsize=20)
