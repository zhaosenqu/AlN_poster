...
def main():
    ...
    # --- Error estimates ---
    # Scherrer grain size: minimum estimate (no instrument broadening subtraction)
    # Assume ~10-15% uncertainty as lower bound error (actual size could be larger)
    coherence_err_lower = cv.coherence_length_nm * 0.0  # No lower error (this is minimum)
    coherence_err_upper = cv.coherence_length_nm * 0.10  # Could be ~20% larger
    coherence_err = np.array([coherence_err_lower, coherence_err_upper])
    
    # c-lattice parameter: typical XRD uncertainty ~0.001 Å
    c_lattice_err = np.full(len(cv), 0.001)  # Adjust based on your actual uncertainty
    ...
    for xi, v, err_up in zip(x, cv.coherence_length_nm, coherence_err_upper):
        a1.text(xi, v + err_up + 1.2, f'{v:.0f}', ha='center', fontsize=16.5, color=INK)
